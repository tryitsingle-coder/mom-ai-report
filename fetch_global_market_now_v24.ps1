  ...
  try {
    if(Test-Path ".\global_market.json"){
      ...
      foreach($it in $old.items){
        if($it.symbol -eq $symbol -and $null -ne $it.pct){ return $it }
      }
    }
  } catch {}
  return $null
...
function Get-YahooPct($symbol) {
  ...
  if($null -ne $old){ return @{ price=$old.price; previous=$old.previous; pct=$old.pct; ok=$true; source="沿用上一筆" } }
...
foreach($x in $items){
  ...
  $resultItems += [pscustomobject]@{
    label = $x.label; symbol = $x.symbol; price = $d.price; previous = $d.previous
    pct = $d.pct; ok = $d.ok; inverse = $x.inverse; source = $d.source
  }
}
...
if($okCount -eq 0){ $status="等待國際盤資料 - 先看 09:08 真方向"; $score=50 }
elseif($score -ge 65){ $status="偏多 - 可觀察回測低接" }
elseif($score -ge 45){ $status="中性 - 等 09:08 真方向" }
else { $status="偏空 - 開低不急接" }
...
function CardClass($pct, $inverse){
  ...
  $p=[double]$pct
  if($inverse){ if($p -gt 1){ return "summary-red" }; if($p -lt -1){ return "summary-green" }; return "summary-gray" }
  else { if($p -gt 1){ return "summary-green" }; if($p -lt -1){ return "summary-red" }; return "summary-yellow" }
}
$cards = ""
...
foreach($i in $resultItems){
  if($null -eq $i.pct){ $pctText = "等待更新" } else { $pctText = ("{0:+0.00;-0.00;0.00}%" -f [double]$i.pct) }
